        ...
        success: function(jsonData){
            ...
            for (var i = 0; i < jsonData.length; i++) {
                ...
                tempItem =  {
                    ...
                    artist: jsonData[i].artist_name, // 艺术家名字
    ...
    $.ajax({
        ...
        success: function(jsonData){
            ...
            var tempList = {
                ...
                creatorName: jsonData.playlist.creator_nickname,   // 列表创建者名字
                creatorAvatar: jsonData.playlist.creator_avatarUrl,   // 列表创建者头像
                item: []
            };
            ...
            if(typeof jsonData.playlist.tracks !== undefined || jsonData.playlist.tracks.length !== 0) {
                ...
                for (var i = 0; i < jsonData.playlist.tracks.length; i++) {
                    tempList.item[i] =  {
                        id: jsonData.playlist.tracks[i].song_id,  // 音乐ID
                        name: jsonData.playlist.tracks[i].song_name,  // 音乐名字
                        artist: jsonData.playlist.tracks[i].artist_name, // 艺术家名字
                        album: jsonData.playlist.tracks[i].album_name,    // 专辑名字
                        source: "netease",     // 音乐来源
                        url_id: jsonData.playlist.tracks[i].url_id,  // 链接ID
                        pic_id: null,  // 封面ID
                        lyric_id: jsonData.playlist.tracks[i].lyric_id,  // 歌词ID
                        pic: jsonData.playlist.tracks[i].album_coverImgUrl + "?param=300y300",    // 专辑图片
                        url: null   // mp3链接
                    };
                }
            }
